...
<template>
  <view class="animal-select" :class="{ 'animal-select--disabled': disabled }">
    <view
      class="animal-select__trigger"
      :class="{ 'animal-select__trigger--disabled': disabled }"
      @click="handleToggle"
    >
      <text class="animal-select__value" :class="{ 'animal-select__value--placeholder': !modelValue }">
        {{ currentLabel }}
      </text>
      <view class="animal-select__arrow" :class="{ 'animal-select__arrow--open': open }" aria-hidden="true" />
    </view>
    <!-- 点击外部关闭：全屏透明遮罩 -->
    <view v-if="open" class="animal-select__mask" @click.stop="close" />
    <view v-if="open" class="animal-select__dropdown">
      <view
