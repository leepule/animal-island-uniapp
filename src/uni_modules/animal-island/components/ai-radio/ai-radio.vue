...
const { value: selectedValue, setValue: setSelectedValue } = useControlled<RadioValue | undefined>(
  () => props.modelValue,
  undefined,
  (value) => {
...
const focusedIndex = ref<number>(
  Math.max(
    0,
    props.options.findIndex((o) => o.value === selectedValue.value)
  )
);
...
<template>
  ...
  >
    ...
    >
      ...
      >
        <view v-if="isChecked(opt.value)" class="animal-radio__mark" aria-hidden="true" />
      </view>
...
.animal-radio {
  ...
  &--middle {
    .animal-radio__circle {
      width: 22px;
      height: 22px;
      border-radius: @border-radius-sm - 4rpx;
    }
    .animal-radio__mark {
      width: 12px;
      height: 12px;
